...
###### DVH config ###################
dvh_vp = False 
dvh_idle = ['N'] * target_level
dvh_idle[0] = 'N'
dvh_ipi = ['N'] * target_level
dvh_timer = ['N'] * target_level
dvh_fs_base = ['N'] * target_level
...
cmd = 'sed -i \'s/kvm-intel.nested=1//\' /etc/default/grub'
child.sendline(cmd)
wait_for_prompt(child, hostnames[1])
child.sendline('update-grub')
wait_for_prompt(child, hostnames[1])
